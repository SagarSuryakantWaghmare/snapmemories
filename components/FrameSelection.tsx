  ...
  const renderFramePreview = (frame: Frame, isSelected: boolean, size: 'small' | 'large' = 'small') => {
    ...
    if (frame.shape === 'heart') {
      return (
        <div className={`${baseClasses} ${sizeClasses}`}>
          ...
          >
            <svg viewBox="0 0 100 100" className={size === 'large' ? 'w-[4.5rem] h-[4.5rem] sm:w-20 sm:h-20' : 'w-12 h-12 sm:w-14 sm:h-14'}>
              <defs>
    ...
    if (frame.shape === 'polaroid') {
      return (
        ...
        >
          <div className={`${size === 'large' ? 'w-16 h-16 sm:w-[4.5rem] sm:h-[4.5rem]' : 'w-10 h-10 sm:w-12 sm:h-12'} bg-gray-200 overflow-hidden`}>
            {photoPreview ? (
              <Image src={photoPreview} alt="Preview" fill className="object-cover" />
            ) : (
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-3 pb-28 sm:pb-32 overflow-hidden">
          <div className="relative w-full max-w-md flex items-center justify-center mb-3">
            <button
              ...
              className="absolute left-0 z-20 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center hover:bg-white active:scale-95"
            ...
            <button
              ...
              className="absolute right-0 z-20 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center hover:bg-white active:scale-95"
