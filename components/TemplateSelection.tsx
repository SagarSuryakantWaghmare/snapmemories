    ...
    <div className="w-full h-full min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 flex flex-col overflow-hidden">
      ...
      <main className="flex h-full min-h-screen flex-col">
        <header className="pt-14 sm:pt-16 pb-4 text-center px-4 animate-fade-in-down">
          <p className="text-[11px] sm:text-xs uppercase tracking-[0.12em] text-gray-500 font-semibold mb-1 opacity-75">Step 1 of 3</p>
          <h1 className="text-2xl sm:text-3xl font-bold text-black mt-1">Choose your style</h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-2">
            Selected: <span className="font-semibold text-gray-700 bg-gray-100 px-2 py-0.5 rounded-full inline-flex mt-1">{selectedTemplate.name}</span> 
          </p>
            ...
            {templates.map((template, index) => {
              ...
              return (
                <button
                  ...
                  className={`relative group bg-white rounded-2xl overflow-hidden border-2 transition-all duration-300 hover:shadow-button-hover active:scale-[0.98] animate-fade-in ${
                    isSelected ? 'border-black shadow-lg scale-100' : 'border-gray-200 hover:border-gray-400'
                  }`}
                ...
                >
                  ...
                  >
                    ...
                    <div className="p-3 border-t border-gray-100 bg-white text-left">
                     <h3 className="font-semibold text-xs sm:text-sm text-black truncate group-hover:text-gray-700 transition-colors">{template.name}</h3>
                     <p className="text-[10px] sm:text-xs text-gray-500 mt-0.5 truncate">{template.description}</p>

                    <div className="flex gap-1.5 mt-2">
                       <div className="w-3 h-3 rounded-full border border-gray-200 hover:scale-125 transition-transform" style={{ background: template.colors.primary }} />
                       <div className="w-3 h-3 rounded-full border border-gray-200 hover:scale-125 transition-transform" style={{ background: template.colors.secondary }} />
                       <div className="w-3 h-3 rounded-full border border-gray-200 hover:scale-125 transition-transform" style={{ background: template.colors.accent }} />
                    </div>
                  </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 px-4 pt-3 pb-4 safe-bottom bg-gradient-to-t from-white via-white/95 to-transparent backdrop-blur-sm border-t border-black/5 shadow-xl">
          <button
            type="button"
            onClick={onContinue}
            className="w-full max-w-md mx-auto block px-8 sm:px-10 py-3 sm:py-3.5 bg-gradient-to-r from-black to-gray-800 text-white text-sm sm:text-base font-bold rounded-full hover:shadow-button-hover active:scale-[0.98] shadow-lg transition-all duration-200 group relative overflow-hidden"
