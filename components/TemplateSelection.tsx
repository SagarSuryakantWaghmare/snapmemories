      ...
      <main className="flex h-full min-h-screen flex-col">
            ...
            {templates.map((template, index) => {
              ...
              return (
                <button
                  ...
                  className={`relative group bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border-2 transition-all duration-300 hover:shadow-button-hover active:scale-[0.98] animate-fade-in ${
                    isSelected ? 'border-black dark:border-white shadow-lg scale-100' : 'border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-600'
                  }`}
                ...
                >
                  ...
                  >
                    <div className="w-full max-w-[80px] flex flex-col gap-0.5">
                      {[1, 2, 3, 4].map((index) => (
                        <div
                          key={index}
                          className="aspect-square bg-gray-200 dark:bg-gray-600 rounded-sm flex items-center justify-center hover:scale-105 transition-transform duration-200 group-hover:shadow-sm"
                          style={{
                  ...
                  <div className="p-3 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 text-left">
                    <h3 className="font-semibold text-xs sm:text-sm text-black dark:text-white truncate group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">{template.name}</h3>
                    <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">{template.description}</p>

                    <div className="flex gap-1.5 mt-2">
                      <div className="w-3 h-3 rounded-full border border-gray-200 dark:border-gray-700 hover:scale-125 transition-transform group-hover:shadow-glow" style={{ background: template.colors.primary }} />
                      <div className="w-3 h-3 rounded-full border border-gray-200 dark:border-gray-700 hover:scale-125 transition-transform group-hover:shadow-glow" style={{ background: template.colors.secondary }} />
                      <div className="w-3 h-3 rounded-full border border-gray-200 dark:border-gray-700 hover:scale-125 transition-transform group-hover:shadow-glow" style={{ background: template.colors.accent }} />
                    </div>
                  </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 px-4 pt-3 pb-4 safe-bottom bg-gradient-to-t from-white via-white/95 to-transparent dark:from-gray-800 dark:via-gray-800/95 dark:to-transparent backdrop-blur-sm border-t border-black/5 dark:border-white/10 shadow-xl">
          <button
            type="button"
            onClick={onContinue}
            className="w-full max-w-md mx-auto block px-8 sm:px-10 py-3 sm:py-3.5 bg-gradient-to-r from-black to-gray-800 dark:from-white dark:to-gray-200 text-white dark:text-black text-sm sm:text-base font-bold rounded-full hover:shadow-button-hover active:scale-[0.98] shadow-lg transition-all duration-200 group relative overflow-hidden"
          ...
          >
            ...
            <span className="absolute inset-0 bg-gradient-to-r from-gray-800 to-black dark:from-gray-200 dark:to-white opacity-0 group-hover:opacity-20 transition-opacity" />
          </button>
        </div>
      </main>
    </div>
